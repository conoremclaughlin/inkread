...
/** The signed-in reader's personal library (private; publishing lives here). */
    ...
    <main className="mx-auto max-w-3xl px-6 py-10">
      <header className="flex items-center justify-between">
        <div className="flex items-baseline gap-3">
          ...
          <Link href="/" className="text-sm text-[#8b5e3c] transition hover:underline">
            Discover
          </Link>
        </div>
      ...
      {books.length === 0 ? (
        ...
        <ul className="mt-8 space-y-3">
          {books.map((book) => (
            <li key={book.id} className="group flex rounded-xl border border-[#e6dfd4] bg-white">
