...
/**
 * PATCH /api/books/:bookId — owner publishes/unpublishes, sets serialization
 * status, and sets the pricing policy (free head + per-chapter coin price).
 */
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    for (const key of ['freeChapterCount', 'coinsPerChapter'] as const) {
    ...
    const hasUpdate =
      ...
      body.coinsPerChapter !== undefined;
    if (!hasUpdate) {
